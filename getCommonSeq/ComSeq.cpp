...
void ComSeq::PreAttributeCluster()
{
    ...
    double AC_sqrt =0;// All the components
    mat U;
    Eigen::VectorXd s;
    Eigen::MatrixXd V;
    mat Kernal;
...

   {
       ...
       if( SampleS.n_cols > 16384 )
       {
           
             int n_rows = SampleS.n_rows;
             int nev = std::min(8192,n_rows);
             Eigen::MatrixXd M;

             {
                Eigen::MatrixXd M;
                int size; 
                Eigen::SparseMatrix <double> Mat (SampleS.n_rows,SampleS.n_cols);
         


               for(arma::sp_mat::const_iterator it = SampleS.begin(); it != SampleS.end(); ++it) {

                int i;
                int j;
                double value; 
                  i = it.row(); // Row position
                  j = it.col(); // Col position
                  value =  *it;    // Value
                  Mat.insert(i,j) = value;
    
               }

               M =  MatrixXd(Mat.transpose()*Mat);
               DenseSymMatProd<double> op(M);
               SymEigsSolver<DenseSymMatProd<double>> eigensolver(op, nev, nev*2);
               eigensolver.init();

               int nconv = eigensolver.compute();
            // Retrieve results

               if(eigensolver.info() != CompInfo::Successful)
               {
       ...
       }else
       {
            ...
            if(eigensolver.info() != Eigen::ComputationInfo())
            {
                 printf("Matrix decomposition failed\n");
                 exit(0);
            }
      ...
      cor_orignal.resize(SampleS.n_cols,Rank);
      ...
      for( int i=0;i<s.size();i++)
      {
        ...
        for(int j=0;j<SampleS.n_cols;j++)
         if(Correlation_Matrix[1,j]!=0)
            Rate[i]= Rate[i] + (V(j,i)*V(j,i))/Correlation_Matrix[1,j];
      }
    ...
    for(int j=0;j< cor.n_cols;j++)
     ...
     Normalization.resize(cor.n_rows);
  ...
  //  U.resize(U.n_rows,Rank);
    ...
    for(int j=0;j<cor.n_cols;j++)
    {
...

        {
            ...
            for(int i=0;i<cor.n_rows;i++)
            {
                Square = Square+cor(i,j)*cor(i,j);
                mean_Sqr = mean_Sqr+(Sum_vector[i]*cor_orignal(i,j));//mean = mean+Sum_vector[i]*cor(i,j);
            }

           // printf("Square:%lf\n",(Square));
            mean_Sqr = Rate[j]*(mean_Sqr*mean_Sqr)/(SampleS.n_rows*SampleS.n_rows);
          //  printf("mean:%lf\n",(mean_Sqr));
          //  printf("Square:%lf\n",(Square/SampleS.n_rows));

           // printf("S:%lf\n",s[s.n_elem-1-j]/(SampleS.n_rows*SampleS.n_rows));
          ...
          //  printf("Mean:%lf\n",mean_Sqr);
           ...
           if(mean_Sqr!=0)
           {
              ...
              double Diff = ((Square/SampleS.n_rows - mean_Sqr)/(s(j)/(SampleS.n_rows*SampleS.n_rows)));
              fprintf(op___,"%lf\n",Diff);
          

              if(Diff<VarThreshold)
              {
                continue;
              }
...

        {
               ...
               {  
                   {
                       Normalization[i]=cor(i,j)*cor(i,j)+Normalization[i];
                   }
               }
        }
    ...
    fclose(op___);
     ...
     for(int i=0;i<CW.size();i++)
     {
            ...
            {
                CW[i][j]=cor(i,j)*cor(i,j)/(Normalization[i]+0.0000000000000000000000001);
            }
     }
